...
- [ ] **Step 1:** Baixar Leaflet 1.9.4 (`curl -L https://unpkg.com/leaflet@1.9.4/dist/...` → `app/vendor/`), incluindo `images/` (marker-icon, marker-shadow).
- [ ] **Step 2:** `index.html`: viewport mobile, `<div id="map">`, header com título da cidade + toggle mapa/lista, container do bottom sheet vazio. Carrega vendor + app.js com `defer`.
- [ ] **Step 3:** `app.js`: init Leaflet (tiles `https://tile.openstreetmap.org/{z}/{x}/{y}.png`, attribution OSM), fetch do tour.json, polyline da `route`, marcadores numerados (divIcon com número da ordem; verde quando visitado), `fitBounds` nos POIs. Erro de fetch/JSON → `<div class="error">` com mensagem legível.
- [ ] **Step 4 (verify):** `python3 -m http.server 8777` na raiz do worktree + Playwright: abrir `http://localhost:8777/app/?city=dubrovnik`, screenshot mostra mapa com 2 marcadores numerados e rota. Console sem erros.
- [ ] **Step 5:** Commit `feat: mapa base com rota e marcadores`.
...
- [ ] **Step 1:** Markup + CSS do sheet (fixo embaixo, ~65vh, fechar via botão ✕; dark-friendly; touch targets ≥44px).
- [ ] **Step 2:** JS: render do sheet a partir do objeto POI; player com `playbackRate`; Media Session API (`title` = nome do POI, `artist` = "Walking Tour — <cidade>"); Wake Lock API request no primeiro play (re-request em `visibilitychange`).
- [ ] **Step 3 (verify):** Playwright: click no marker 1 → sheet abre com foto e badge "áudio em preparo" (stub sem áudio); click "Visitado" → marker fica verde e persiste após reload.
- [ ] **Step 4:** Commit `feat: painel de POI com fotos, player e visitado`.
...
- [ ] **Step 1:** Toggle mapa/lista no header (estado em localStorage `wt-view`).
- [ ] **Step 2:** Render da lista + distâncias; click abre `openPoi`.
- [ ] **Step 3:** Geolocalização com tratamento de erro (denied → toast "GPS indisponível — use o mapa").
- [ ] **Step 4 (verify):** Playwright: alternar para lista, ver distâncias plausíveis entre os 2 POIs stub (Pile→Onofrio ≈ 100–200 m); console limpo.
- [ ] **Step 5:** Commit `feat: modo lista com distâncias e GPS opcional`.
...
- [ ] **Step 1 (TDD):** `test_gerar_precache.py`: (a) tile x/y correto para lat 42.6414, lng 18.1064 em z16 (comparar com fórmula de referência no teste); (b) bbox com margem inclui tiles vizinhos; (c) contagem de tiles z15–17 da Cidade Velha < 400. Rodar: FAIL (módulo não existe).
- [ ] **Step 2:** Implementar `gerar_precache.py`; rodar `python3 -m pytest pipeline/ -q`: PASS.
- [ ] **Step 3:** Ícones 192/512 px (fundo sólido + "WT", via ffmpeg lavfi/drawtext ou sips). Manifest: `display: standalone`, `start_url` relativo correto no Pages.
- [ ] **Step 4:** `sw.js` + registro no app.js; `app/cities.json` = `["dubrovnik"]`.
- [ ] **Step 5 (verify):** Playwright: 1º load com rede → 2º load com `context.setOffline(true)` → app renderiza mapa (tiles do cache) e abre POI.
- [ ] **Step 6:** Commit `feat: PWA offline (manifest, service worker, precache de tiles)`.
...
- [ ] **Step 1:** Subagents de pesquisa (paralelos): (a) fatos históricos + 1 curiosidade por POI com fontes; (b) coordenadas precisas de cada POI + info prática 2026 (preços/horários de Muralhas, Lovrijenac, Palácio do Reitor, Farmácia do Mosteiro). POIs: Pile Gate, Fonte Grande de Onofrio, Mosteiro Franciscano/Farmácia, Stradun, Palácio Sponza, Igreja de São Brás, Palácio do Reitor, Catedral, Praça Gundulić, Buža/muralhas sul, Forte Lovrijenac.
- [ ] **Step 2:** Escrever os 11 scripts PT-BR (150–250 palavras; estrutura: gancho → história → curiosidade → "o que olhar agora"; UMA FRASE POR LINHA — requisito do pipeline de áudio) + intro de ~30 s. Escrever `practical` de cada POI.
- [ ] **Step 3:** Montar `route` (traçado Pile→…→Lovrijenac) e `maps_url` por POI.
- [ ] **Step 4 (verify):** JSON válido (`python3 -m json.tool`); contagem de palavras de cada script dentro de 150–250; coords dentro da bbox de Dubrovnik (42.63–42.65 / 18.10–18.12).
- [ ] **Step 5:** Commit `feat: tour de Dubrovnik (11 POIs, textos e rota)`.
...
- [ ] **Step 1:** Implementar e rodar `baixar_fotos.py` para os 11 POIs (termos de busca por POI num dicionário no script).
- [ ] **Step 2 (curadoria obrigatória):** Read de CADA imagem baixada — conferir que mostra o POI certo e tem qualidade; descartar erradas/ruins e rebuscar. Redimensionar finais para ≤1280px (`sips -Z 1280`).
- [ ] **Step 3:** Preencher `photos[]` no tour.json (caption PT-BR + credit da licença).
- [ ] **Step 4 (verify):** todo POI com ≥1 foto; todos os arquivos referenciados existem; peso total `photos/` < 15 MB.
- [ ] **Step 5:** Commit `feat: fotos dos POIs (Wikimedia Commons, com créditos)`.
...
- [ ] **Step 1 (TDD):** testes com tour mínimo válido e casos inválidos (foto faltando, order duplicado, script curto). FAIL.
- [ ] **Step 2:** Implementar; `python3 -m pytest pipeline/ -q`: PASS.
- [ ] **Step 3:** Rodar no dubrovnik real: PASS. Corrigir o que apontar.
- [ ] **Step 4:** Commit `feat: validador de tour + verde no dubrovnik`.
...
- [ ] **Step 1:** Regenerar `precache.json` (fotos novas), bump versão do SW.
- [ ] **Step 2:** Criar repo e publicar: `gh repo create willfgo/walking-tour --public` + remote + `git push -u origin dev:main` (a partir do worktree `dev`).
- [ ] **Step 3:** Ativar Pages: `gh api repos/willfgo/walking-tour/pages -X POST -f "source[branch]=main" -f "source[path]=/"` (PUT se já existir). Aguardar `status: built` com loop `until` (sem `timeout` neste macOS).
- [ ] **Step 4 (verify):** `curl -sI https://willfgo.github.io/walking-tour/app/` → 200; Playwright contra a URL pública: mapa, POI, foto, lista; offline após 1º load.
- [ ] **Step 5:** Avisar o usuário (mensagem + PushNotification): URL, instrução "abrir no Safari → Compartilhar → Adicionar à Tela de Início; abrir 1× no wifi antes de sair".
...
- [ ] **Step 1:** `cd ~/Desktop/Claude/omnivoice && /opt/homebrew/opt/python@3.12/bin/python3.12 -m venv venv && venv/bin/pip install omnivoice` (cache pip de 1 GB + modelo HF de 3 GB já locais; wifi de hotel cobre só diffs).
- [ ] **Step 2 (smoke):** sintetizar 1 frase teste com `--device mps --language pt` e o `ref-animada.wav`; conferir wav gerado e duração plausível (ffprobe). Se MPS falhar → diagnosticar com `--device cpu`, mas NÃO usar cpu para a fila (lento demais).
- [ ] **Step 3:** Registrar tempo/frase medido para estimar a fila.
...
- [ ] **Step 1 (TDD):** testes das partes puras com runner fake (padrão do voz.py): divisão de frases ignora vazias/#; POI com falha não escreve `audio`; nome do mp3 = `NN-slug.mp3`. FAIL→PASS.
- [ ] **Step 2:** Implementar (reusar padrões de `~/.claude/scripts/podcast-matinal/voz.py`, MENOS o fallback say).
- [ ] **Step 3 (smoke):** gerar só a intro (~30 s) de ponta a ponta; validar mp3 no ffprobe e tocar no app local.
- [ ] **Step 4:** Commit `feat: pipeline de áudio OmniVoice`.
